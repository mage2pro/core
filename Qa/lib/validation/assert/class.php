 ...
 * 3) Если $v не является объектом, то функция просто вернёт `false`.
 * 4) Наша функция не загружает при этом $cc в память интерпретатора PHP.
 * 5) Если $cc ещё не загружен в память интерпретатора PHP, то функция вернёт `false`.
 * 5.1) Это логично: если проверяемый класс ещё не был загружен в память интерпретатора PHP,
 * то проверяемая переменная $v гарантированно не может принадлежать данному классу.
 * http://3v4l.org/KguI5
 * 6) Наша функция отличается по сфере применения как от оператора `instanceof`, так и от функции @see is_a() тем, что:
 * 6.1) Умеет проводить проверку на приналежность не только одному конкретному классу, а и хотя бы одному из нескольких.
 * 6.2) @see is_a() приводит к предупреждению уровня `E_DEPRECATED` интерпретатора PHP версий ниже 5.3:
